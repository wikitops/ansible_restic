...
SUPPORTED_OS = {
  "debian"   => {box: "debian/buster64", bootstrap_os: "debian", user: "vagrant"},
  "ubuntu"   => {box: "ubuntu/focal64",  bootstrap_os: "ubuntu", user: "vagrant"},
    ...
    config.vm.define vm_name = "%s%02d" % [$instance_name_prefix, i] do |server|
      ...
      if i == $num_instances
        server.vm.provision "ansible" do |ansible|
          ...
          ansible.groups = {
            "restic" => ["#{$instance_name_prefix}0[1:#{$num_instances}]"]
          }
        end
      end
    end
